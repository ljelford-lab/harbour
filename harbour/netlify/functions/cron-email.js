// Runs automatically on the schedule below — Netlify Scheduled Functions
// can't be triggered by a public URL (unlike a normal function), so there's
// no separate secret needed to protect this one from outside callers.
// You can still trigger it manually from the Netlify UI (Functions tab →
// this function → "Run now") to test it without waiting for the schedule.
const { schedule } = require('@netlify/functions');
const { Resend } = require('resend');
const { getWeek } = require('./lib/_store');
...
// Runs at 06:00 UTC daily — see the README for the BST/GMT caveat.
exports.handler = schedule('0 6 * * *', task);
